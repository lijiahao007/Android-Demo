...
public class ElasticBallView extends View {
    ...
    public ElasticBallView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public ElasticBallView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init();
    }

    public ElasticBallView(Context context, @Nullable AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private void anim() {
        valueAnimator = ValueAnimator.ofFloat(0, 1.0f); // 属性动画
        valueAnimator.setInterpolator(new AccelerateDecelerateInterpolator()); // 插值器
        valueAnimator.setDuration(DURATION);
        valueAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            @Override
            public void onAnimationUpdate(ValueAnimator animation) { // 当动画的属性更新时调用
                fraction = animation.getAnimatedFraction(); // 取值范围是0~1, 从动画开始到结束(从0~1)
                Log.i("ElasticBallView", "fraction:" + fraction);
                invalidate(); // 重绘
            }
        });
        valueAnimator.start();
